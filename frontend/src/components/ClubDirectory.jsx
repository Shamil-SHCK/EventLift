...
const ClubDirectory = ({ hideLayout = false }) => {
    ...
    useEffect(() => {
        const initData = async () => {
            try {
                // Fetch clubs and current user in parallel to know if we need to filter
                const [data, currentUser] = await Promise.all([
                    fetchClubsDirectory(),
                    getCurrentUser().catch(() => null) // Ignore error if not logged in
                ]);
                ...
                if (currentUser && (currentUser.role === 'alumni' || currentUser.role === 'alumni-individual')) {
                    ...
                    if (formerInstitution) {
                        filtered = data.filter(club =>
                            club.collegeName && club.collegeName.toLowerCase() === formerInstitution.toLowerCase()
                        );
                    }
                }
