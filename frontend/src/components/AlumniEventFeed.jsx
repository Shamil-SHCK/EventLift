...
const AlumniEventFeed = ({ onSponsorshipSuccess, user }) => {
    ...
    useEffect(() => {
        ...
        // Filter by Alumni's former institution
        const formerInstitution = user?.profile?.formerInstitution || user?.formerInstitution;
        if (formerInstitution) {
            result = result.filter(e => {
                const eventCollege = e.organizer?.collegeName;
                if (!eventCollege) return false;
                return eventCollege.toLowerCase() === formerInstitution.toLowerCase();
            });
        }
                            ...
                            <div className="mt-auto">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button
                                        ...
                                        className="col-span-2 px-4 py-2.5 rounded-xl bg-amber-500 text-white font-semibold text-sm hover:bg-amber-600 transition-colors shadow-lg shadow-amber-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
